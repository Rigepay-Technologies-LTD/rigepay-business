...
async function load() {
  ...
  error.value = null
  try {
    schedules.value = await fetchInvoiceSchedules()
  } catch (err) {
...
async function handlePause(s: InvoiceSchedule) {
  ...
  try {
    await patchInvoiceSchedule(s.id, { action: 'pause' })
    await load()
  } catch (err) {
...
async function handleResume(s: InvoiceSchedule) {
  actionLoading.value = s.id
  try {
    await patchInvoiceSchedule(s.id, { action: 'resume' })
    await load()
  } catch (err) {
    showError(extractErrorMessage(err))
  } finally {
...
async function handleCancel(s: InvoiceSchedule) {
  ...
  actionLoading.value = s.id
  try {
    await patchInvoiceSchedule(s.id, { action: 'cancel' })
    await load()
  } catch (err) {
    showError(extractErrorMessage(err))
  } finally {
...
<template>
  <DashboardLayout :org-id="props.orgId" title="Invoice schedules">
    <div class="flex flex-col gap-6">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-sm font-bold text-text-primary">Recurring invoices</h2>
          <p class="text-xs text-text-muted mt-0.5">
            Automatically create and send this recipient list every month. Paused automatically if your wallet
            balance can't cover a run — you'll be notified.
          </p>
        </div>
