...
async function load() {
  loading.value = true
  try {
    const [prefs, codes] = await Promise.all([fetchOrgSettlementPreferences(), fetchOrgBankCodes(false)])
...
async function validateShortcodeDestination() {
  ...
  try {
    const result = await validateOrgShortcode(false, shortcode.value.trim(), destinationType.value === 'PAYBILL' ? 'paybill' : 'till')
...
async function submitSave() {
  ...
  if (!/^\d{4}$/.test(pin.value)) {
    formError.value = 'Enter your 4-digit transaction PIN to confirm this change.'
    return
  }
...
async function submitOtp() {
  ...
  try {
    await confirmOrgSettlementPreferences(otp.value)
    otpStep.value = false
    showSuccess('Default settlement destination updated.')
    await load()
    editing.value = false
...
<template>
  <DashboardLayout :org-id="props.orgId" title="Settlement preferences">
    <div class="flex flex-col gap-6 max-w-2xl">
      <div>
        <h1 class="text-lg font-bold text-text-primary">Settlement preferences</h1>
        <p class="text-sm text-text-muted mt-0.5">
          Where auto-settlement rules for this organization send money by default. Individual scheduled payout
          rules can still specify their own destination.
        </p>
      </div>
      ...
      <template v-else>
        ...
        <AppCard v-else-if="editing">
          ...
          <p class="text-xs text-text-muted mb-4">
            Changes are confirmed with your transaction PIN and a one-time code.
          </p>
        ...
        <div class="flex items-start gap-2 text-xs text-text-muted">
          ...
          <p>
            Settlement runs move cleared funds from your MAIN wallet to this destination on the schedule set by
            your auto-settlement rules. High-risk-rail collections are held for 72h before they become eligible.
          </p>
        </div>
      </template>
    </div>
