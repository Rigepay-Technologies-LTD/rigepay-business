...
async function submitAdd() {
  ...
  adding.value = true
  try {
    await createRecipientSuppression({ email: email.value.trim(), reason: reason.value.trim() || undefined })
    showSuccess(`${email.value.trim()} will no longer receive invoices from you.`)
    email.value = ''
    reason.value = ''
    await load()
  } catch (err) {
...
async function handleRemove(s: RecipientSuppression) {
  const ok = await confirmAction({
    ...
    message: `${s.email} will be able to receive invoices from you again — including from any bulk batch or recurring schedule that lists them.`,
  ...
  removing.value = s.id
  try {
    await deleteRecipientSuppression(s.email)
...
<template>
  <DashboardLayout :org-id="props.orgId" title="Recipient suppressions">
    <div class="flex flex-col gap-6">
      <div>
        ...
        <p class="text-xs text-text-muted mt-0.5">
          ...
          schedule, checked again at the moment each invoice would actually send.
        </p>
      </div>
      ...
      <AppCard>
        ...
        <form class="flex flex-col sm:flex-row gap-3 items-end" @submit.prevent="submitAdd">
          <AppInput v-model="email" label="Email" placeholder="jane@example.com" class="flex-1" required />
      ...
      <AppCard v-else-if="!suppressions.length" padding="lg">
        <div class="flex flex-col items-center text-center gap-2 py-6">
          ...
          <p class="text-xs text-text-muted">Every recipient in your bulk batches and schedules currently receives invoices normally.</p>
        </div>
      </AppCard>
