...
async function load() {
  ...
  error.value = null
  try {
    batches.value = await fetchInvoiceBatches()
  } catch (err) {
...
async function runEstimate() {
  ...
  try {
    estimate.value = await estimateBulkInvoices({ recipients })
  } catch (err) {
...
async function submitSend() {
  ...
  sending.value = true
  try {
    const result = await sendBulkInvoices({ recipients })
...
async function toggleExpand(batchId: string) {
  ...
  try {
    batchDetail.value = await fetchInvoiceBatch(batchId)
  } catch (err) {
    const msg = extractErrorMessage(err)
    error.value = msg
    showError(msg)
...
<template>
  <DashboardLayout :org-id="props.orgId" title="Bulk invoices">
    <div class="flex flex-col gap-6">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-sm font-bold text-text-primary">Send invoices in bulk</h2>
          <p class="text-xs text-text-muted mt-0.5">
            Add a list of recipients — the full batch cost is escrowed immediately, then each invoice is generated
            and emailed independently. KES 1 per invoice actually sent (suppressed/failed recipients aren't charged).
          </p>
        </div>
      ...
      <AppCard v-if="showCreateForm" padding="none">
        ...
        <div class="px-5 pb-5 pt-4 flex flex-col gap-4">
          ...
          <div v-if="estimate" class="text-xs text-text-secondary bg-surface-2 rounded-lg px-3 py-2 flex flex-col gap-1">
            ...
            <span>Wallet balance: KES {{ formatMoney(estimate.wallet_balance_cents) }}
