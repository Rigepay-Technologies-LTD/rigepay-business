...
async function load() {
  ...
  try {
    const [t, b] = await Promise.all([
      fetchTags(),
      fetchTagBreakdown({ from: fromDate.value, to: toDate.value }),
    ])
    tags.value = t
    breakdown.value = b
  } catch (err) {
...
async function reloadBreakdown() {
  try {
    breakdown.value = await fetchTagBreakdown({ from: fromDate.value, to: toDate.value })
  } catch (err) {
    const msg = extractErrorMessage(err)
    error.value = msg
    showError(msg)
...
<template>
  <DashboardLayout :org-id="props.orgId" title="Tags & spend breakdown">
    <div class="flex flex-col gap-6">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-sm font-bold text-text-primary">Tags & spend breakdown</h2>
          <p class="text-xs text-text-muted mt-0.5">Categorize payouts and expenses to see what you're actually spending on.</p>
        </div>
      ...
      <AppCard>
        <h3 class="text-sm font-bold text-text-primary mb-3">Your tags</h3>
