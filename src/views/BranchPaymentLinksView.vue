...
import {
  fetchBranchPaymentLinks, createBranchPaymentLink, closeBranchPaymentLink,
  type OrgPaymentLink,
} from '@/lib/orgApi'
...
async function load() {
  ...
  error.value = null
  try {
    links.value = await fetchBranchPaymentLinks()
  } catch (err) {
...
async function submitCreate() {
  ...
  creating.value = true
  try {
    const result = await createBranchPaymentLink({
    ...
    showSuccess(`Link created: ${result.url}`)
    amountKes.value = ''
    description.value = ''
...
async function handleClose(link: OrgPaymentLink) {
  ...
  try {
    await closeBranchPaymentLink(link.id)
    await load()
  } catch (err) {
    const msg = extractErrorMessage(err)
    error.value = msg
...
<template>
  <DashboardLayout :org-id="props.orgId" :branch-id="props.branchId" title="Payment links">
    <div class="flex flex-col gap-6">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-sm font-bold text-text-primary">Payment links</h2>
          <p class="text-xs text-text-muted mt-0.5">
            Shareable links people can pay via M-Pesa STK, paybill, bank transfer, or card — paid into this
            branch's own wallet. Hosted at pay.rigepay.co.ke.
          </p>
        </div>
      ...
      <AppCard v-if="showCreateForm">
        ...
        <form class="flex flex-col gap-4 max-w-sm" @submit.prevent="submitCreate">
          ...
          />
          <AppInput v-model="description" label="Description (optional)" placeholder="e.g. Table booking deposit" />
          <label class="flex items-center gap-2 text-sm font-medium text-text-primary">
