...
async function submitCreate() {
  ...
  creating.value = true
  try {
    await createOrgSupportTicket(newSubject.value.trim(), newMessage.value.trim(), false)
    newSubject.value = ''
    newMessage.value = ''
...
async function refreshDetail() {
  ...
  try {
    selectedTicket.value = await fetchOrgSupportTicket(selectedTicket.value.id, false)
  } catch {
...
async function submitReply() {
  ...
  try {
    await sendOrgSupportMessage(selectedTicket.value.id, replyMessage.value.trim(), false)
    replyMessage.value = ''
    await refreshDetail()
  } catch (err) {
    const msg = extractErrorMessage(err)
...
<template>
  <DashboardLayout :org-id="props.orgId" title="Support">
