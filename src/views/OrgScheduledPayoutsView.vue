...
async function submitCreate() {
  ...
  if (sweepFullBalance.value) {
    if (triggerType.value === 'SCHEDULE' && (scheduleType.value !== 'RECURRING' || fundingSource.value !== 'MAIN')) {
      createError.value = 'Auto-settlement (sweep full balance) requires a recurring schedule funded from the MAIN wallet.'
      return
    }
    if (fundingSource.value !== 'MAIN') {
      createError.value = 'Sweep entire balance is only supported from the MAIN wallet.'
      return
    }
  ...
  if (!/^\d{4}$/.test(pin.value)) {
    createError.value = 'Enter your 4-digit transaction PIN to confirm this schedule.'
    return
  }
          ...
          <div class="border-t border-border pt-4 flex flex-col gap-4">
            ...
            <template v-if="triggerType === 'THRESHOLD'">
              ...
              <p class="text-xs text-text-muted -mt-2">
                Fires automatically once the funding wallet's balance reaches this amount, then re-arms only after the
                balance drops back below it and rises again.
              </p>
            </template>
