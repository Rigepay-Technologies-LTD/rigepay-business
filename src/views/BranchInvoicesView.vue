...
async function load() {
  ...
  error.value = null
  try {
    invoices.value = await fetchBranchInvoices()
  } catch (err) {
...
async function handleSend(invoice: OrgInvoice) {
  ...
  try {
    await sendOrgInvoice(invoice.id, channels, true)
    await load()
  } catch (err) {
    const msg = extractErrorMessage(err)
    error.value = msg
...
async function handleDownload(invoice: OrgInvoice) {
  ...
  try {
    const blob = await downloadOrgInvoicePdf(invoice.id, true)
...
<template>
  <DashboardLayout :org-id="props.orgId" :branch-id="props.branchId" title="Invoices">
