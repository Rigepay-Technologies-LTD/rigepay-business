...
async function verifyPayoutShortcode() {
  ...
  try {
    const result = await validateOrgShortcode(true, payoutShortcode.value.trim(), payoutDestinationType.value === 'PAYBILL' ? 'paybill' : 'till')
...
async function verifyPayoutRecipient() {
  ...
  try {
    if (payoutDestinationType.value === 'BANK_ACCOUNT') {
      ...
      const result = await validateOrgMobileMoney(payoutPhoneNumber.value.trim(), undefined, true)
