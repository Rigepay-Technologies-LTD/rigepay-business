  ...
  <DashboardLayout :org-id="props.orgId" title="Audit log">
    ...
    <AppModal v-model="showDetail" title="Activity detail" size="md">
      ...
      <dl v-else-if="selectedDetail" class="flex flex-col gap-3 text-sm">
        ...
        <div class="flex justify-between"><dt class="text-text-muted">Actor</dt><dd class="font-semibold text-text-primary">{{ selectedDetail.actor.type }} ({{ selectedDetail.actor.role }})</dd></div>
        ...
        <div class="flex justify-between"><dt class="text-text-muted">Status</dt><dd><AppBadge :variant="statusVariant(selectedDetail.status)" size="sm">{{ selectedDetail.status }} ({{ selectedDetail.status_code }})</AppBadge></dd></div>
        <div class="flex justify-between"><dt class="text-text-muted">Duration</dt><dd class="font-semibold text-text-primary">{{ selectedDetail.duration_ms }}ms</dd></div>
        ...
        <div v-if="selectedDetail.city || selectedDetail.country" class="flex justify-between"><dt class="text-text-muted">Location</dt><dd class="font-semibold text-text-primary">{{ selectedDetail.city }} {{ selectedDetail.country }}</dd></div>
        <div class="flex justify-between"><dt class="text-text-muted">Occurred</dt><dd class="font-semibold text-text-primary">{{ formatDate(selectedDetail.occurred_at) }}</dd></div>
